...
window.smartBlazor = {
	...
	ensureInstance: function (id, oldId) {
		...
		instances[id].propertyChanged = function (propertyName, oldValue, newValue) {
			if ((instances[id] && instances[id].isRendered) || isCreating) {
		...
		instances[id].initTimer = setInterval(() => {
			...
			const clear = function () {
				if (instances[id]) {
					if (instances[id].nodeName.toLowerCase() !== 'smart-chart') {
	...
	initComponent: function (id, oldId, mappings, properties, dotNetObject) {
		...
		for (let propertyName in componentProperties) {
			if (propertyName.indexOf('Callback') >= 0) {
				const callbackName = propertyName.substring(0, propertyName.indexOf('Callback'));
				callbacks[callbackName] = componentProperties[propertyName];
		...
		for (let callbackName in callbacks) {
			that.addCallbackHandler(id, callbackName, callbacks[callbackName], dotNetObject);
		}
		...
		instances[id].addEventListener(eventName, function (event) {
			try {
				...
				if (normalizedEvent.detail) {
					...
					if (normalizedEvent.detail.parent) {
						normalizedEvent.detail.parent = normalizedEvent.detail.parent.id;
					}
					
					if (normalizedEvent.detail.children) {
						normalizedEvent.detail.children = normalizedEvent.detail.children.map((child) => {
							return child.id;
						})
					}
					
					if (normalizedEvent.detail.data) {
						let data = {};
						for (let key in normalizedEvent.detail.data) {
						
							if (key === 'parent' || key === 'children' || key === '$'){
								continue;
							}
							
							data[key] = normalizedEvent.detail.data[key];
						}
						normalizedEvent.detail.data = data;
					}
